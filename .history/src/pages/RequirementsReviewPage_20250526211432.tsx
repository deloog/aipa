  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 2. 使用 Grid 组件创建双栏布局 */}
        <Grid // 之前可能是 <Grid xs={12} sm={4} md={3}> 或 <Grid item xs={12} sm={4} md={3}>
            // 现在修改为使用 sx 属性定义宽度
            sx={(theme) => ({ // 1. 使用函数形式的sx，可以访问theme对象
              width: '100%', // 默认宽度 (对应 xs={12})
              [theme.breakpoints.up('sm')]: { // 2. 当屏幕宽度达到 'sm' (small) 及以上时
                width: `${(4 / 12) * 100}%`, // 对应 sm={4} (1/3 宽度)
              },
              [theme.breakpoints.up('md')]: { // 3. 当屏幕宽度达到 'md' (medium) 及以上时
                width: `${(3 / 12) * 100}%`, // 对应 md={3} (1/4 宽度)
              },
            })}
          >
            <Paper elevation={1} sx={{ height: '100%', p: 1.5, overflowY: 'auto' }}>
          ...
          {/* 右侧：主内容区 */}
          <Grid // 之前可能是 <Grid xs={12} sm={8} md={9}> 或 <Grid item xs={12} sm={8} md={9}>
            // 现在修改为使用 sx 属性定义宽度
            sx={(theme) => ({
              width: '100%', // 默认宽度 (对应 xs={12})
              [theme.breakpoints.up('sm')]: { // 当屏幕宽度达到 'sm' (small) 及以上时
                width: `${(8 / 12) * 100}%`, // 对应 sm={8} (2/3 宽度)
              },
              [theme.breakpoints.up('md')]: { // 当屏幕宽度达到 'md' (medium) 及以上时
                width: `${(9 / 12) * 100}%`, // 对应 md={9} (3/4 宽度)
              },
            })}
          >
            <Paper elevation={1} sx={{ height: '100%', p: 2, overflowY: 'auto' }}>
