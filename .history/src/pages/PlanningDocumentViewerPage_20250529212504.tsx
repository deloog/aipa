...
import type { FullProjectPlan } from '../types/project.types'; // 导入服务和类型
...
import type { PlanningDocument, Chapter, DocumentPart } from '../types/project.types'; // 导入 PlanningDocument 等类型


// Removed local Chapter, DocumentPart, and PlanningDocument interface declarations as they are now imported
...
const PlanningDocumentViewerPage: React.FC = () => {
  ...
  const locationState = location.state as PlanningPageLocationState | undefined; // This might be less relevant if fetching data directly

  // currentDocumentData and currentProjectName will now be derived from projectPlan state
  // const currentDocumentData: PlanningDocument = ...
  ...
  // Adapt currentDocumentData and currentProjectName based on fetched projectPlan
  // This is a simplified adaptation. You'll need to map FullProjectPlan to PlanningDocument structure
  // or update the component to use FullProjectPlan directly.
  const currentDocumentData: PlanningDocument = projectPlan ? 
    projectPlan.documentData // Directly use documentData from the fetched projectPlan
    : {
        // Fallback for when projectPlan is null (e.g., during initial loading or if an error occurs)
        title: `项目规划文档 (${projectId || '未知项目'})`,
        // Ensure fallback provides required fields for PlanningDocument type from project.types.ts
        requirementsSpec: { title: '需求规格 (加载中...)', chapters: [] }, 
        technicalPlan: { title: '技术规划 (加载中...)', chapters: [] },
      };
  const currentProjectName = projectPlan?.projectName || projectId || '项目';
  ...
  useEffect(() => {
    // This effect now depends on projectPlan (which contains currentDocumentData)
    // It should run when projectPlan is successfully fetched and populated
    if (projectPlan && !loading) { // Ensure data is loaded
  ...
  // Update dependencies: now depends on projectPlan, loading, locationState
  }, [projectPlan, loading, locationState]);  
  ...
  // This effect also depends on projectPlan (which contains currentDocumentData)
  }, [selectedChapterId, projectPlan]); // Changed currentDocumentData to projectPlan
    ...
    parts.forEach(part => {
      ...
      part.chapters.forEach(chapter => { 
        ...
        if (typeof chapter.content === 'string') {
          contentString = chapter.content.charCodeAt(0) === 0xFEFF ? chapter.content.substring(1).trim() : chapter.content.trim();
        } else if (chapter.content !== null && typeof chapter.content === 'object') {
          contentString = JSON.stringify(chapter.content, null, 2);
        } else {
          contentString = String(chapter.content); // Fallback for other types
        }
  ...
  if (loading) return <Typography>正在加载项目规划详情...</Typography>;
  if (error) return <Typography color="error">{error}</Typography>;
  if (!projectPlan) return <Typography>未找到项目规划详情。</Typography>;

  // The rest of the component will use projectPlan or currentDocumentData derived from it
                ...
                <>
                  ...
                  {selectedChapter.id === 'tech_dev_steps' ? (
                    (() => {
                      try {
                        let devStepsData;
                        if (typeof selectedChapter.content === 'object' && selectedChapter.content !== null) {
                          devStepsData = selectedChapter.content;
                        } else if (typeof selectedChapter.content === 'string') {
                          let contentToParse = selectedChapter.content;
                          // Remove BOM if present (UTF-8 BOM is EF BB BF, char code 0xFEFF)
                          if (contentToParse.charCodeAt(0) === 0xFEFF) {
                            contentToParse = contentToParse.substring(1);
                          }
                          contentToParse = contentToParse.trim(); // Trim whitespace

                          if (contentToParse === "") { // Handle empty string case after trim
                            throw new Error("Content string is empty after trimming.");
                          }
                          
                          devStepsData = JSON.parse(contentToParse);
                          
                          // If the first parse results in a string, it might be double-encoded JSON.
                          // Try parsing that string again.
                          if (typeof devStepsData === 'string') {
                            devStepsData = JSON.parse(devStepsData);
                          }
                        } else {
                          throw new Error(`Unexpected content type for tech_dev_steps: ${typeof selectedChapter.content}`);
                        }

                        // Ensure devStepsData is an object, as parsing "null" or "true" is valid JSON but not what we want here.
                        if (typeof devStepsData !== 'object' || devStepsData === null) {
                            throw new Error(`Parsed content is not an object: ${JSON.stringify(devStepsData)}`);
                        }

                        return (
                          <Box>
                            {devStepsData.introduction && (
                            ...
                            {devStepsData.tasks && Array.isArray(devStepsData.tasks) && (
                              <List dense>
                                {devStepsData.tasks.map((task: { taskId: string; title: string; description: string; }, index: number) => (
                                  <ListItem key={task.taskId || index} divider sx={{ alignItems: 'flex-start' }}>
                                    <Box sx={{ width: '100%'}}>
                        ...
                        console.error("Error processing tech_dev_steps content:", e, "Raw content:", selectedChapter.content);
                        // 如果解析失败，仍然尝试显示原始content，但提示错误
                        return (
                          <>
                            <Typography color="error" paragraph>
                              开发步骤内容格式错误，无法正确解析。
                            </Typography>
                            <Typography variant="body1" paragraph sx={{ whiteSpace: 'pre-line', fontFamily: 'monospace', backgroundColor: 'grey.100', p:1, borderRadius:1 }}>
                              {typeof selectedChapter.content === 'string' ? selectedChapter.content : JSON.stringify(selectedChapter.content, null, 2)}
                            </Typography>
                          </>
                        );
                      }
                    ...
                    <Typography variant="body1" paragraph sx={{ whiteSpace: 'pre-line' }}>
                      {(() => {
                        if (typeof selectedChapter.content === 'string') {
                          let contentStr = selectedChapter.content;
                          // Remove BOM if present (UTF-8 BOM is EF BB BF, char code 0xFEFF)
                          if (contentStr.charCodeAt(0) === 0xFEFF) {
                            contentStr = contentStr.substring(1);
                          }
                          return contentStr.trim(); // Trim whitespace
                        } else if (selectedChapter.content !== null && typeof selectedChapter.content === 'object') {
                          return JSON.stringify(selectedChapter.content, null, 2);
                        }
                        else {
                          return String(selectedChapter.content); // Fallback for other types like null, undefined, number, boolean
                        }
                      })()}
                    </Typography>
                  )}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="confirm-dialog-description">
            这份需求规格说明书一旦最终确认，它将成为我们接下来为您进行所有“项目技术规划”的唯一且核心的依据。请您仔细核对。
            后续技术方案都将严格围绕这份已确认的需求来展开。
            <br /><br />
