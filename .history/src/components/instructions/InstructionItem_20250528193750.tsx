...
import type{ AtomizedInstruction } from './types'; // 导入共享的类型定义
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        ...
        <FormControlLabel
          control={
            <Checkbox
              checked={isComplete}
              onChange={() => onToggleComplete(instruction.stepNumber)}
...
{instruction.notes && (
  <Box sx={{ mt: 1, fontStyle: 'italic' }}>
    <Typography variant="caption" color="text.secondary">
      备注：
      {typeof instruction.notes === 'string' 
        ? instruction.notes 
        : (
          <span style={{ color: 'red' }}>
            [无效备注格式] {JSON.stringify(instruction.notes)}
          </span>
        )}
    </Typography>
  </Box>
